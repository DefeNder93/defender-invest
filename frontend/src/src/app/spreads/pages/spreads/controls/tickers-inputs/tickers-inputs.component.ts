...
export class TickersInputsComponent implements OnInit {
  ...
  ngOnInit() {
    this.filteredLeg1Items = combineLatest([
      this.tickers$,
      this.leg1Control.valueChanges.pipe(startWith('')),
    ]).pipe(map(([tickers, value]) => this._filter(tickers, value || '')));
    this.filteredLeg2Items = combineLatest([
      this.tickers$,
      this.leg2Control.valueChanges.pipe(startWith('')),
    ]).pipe(map(([tickers, value]) => this._filter(tickers, value || '')));
  ...
  add = () => {
    this.leg1Control.value &&
      this.leg2Control.value &&
