...
export class CloseChartComponent implements OnInit, OnChanges {
  ...
  private update = (spreads: any | null = null) => {
    ...
    const x = techan.scale.financetime().range([0, width]).outerPadding(0);

    const y = d3.scaleLinear().range([height, 0]);

    const close = techan.plot.close().xScale(x).yScale(y);
    ...
    const tradearrow = techan.plot
      .tradearrow() // http://bl.ocks.org/andredumas/3c0eefdd77a6380b1a35
      .xScale(x)
      .yScale(y)
      .orient(function (d: any) {
  ...
  ngOnChanges(changes: SimpleChanges): void {
    !!changes.spreads.currentValue &&
      !!changes.spreads.currentValue.length &&
      this.update(changes.spreads.currentValue);
  }
}
